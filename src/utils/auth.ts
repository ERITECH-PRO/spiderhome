...
export const logLoginAttempt = async (attempt: LoginAttempt) => {
  try {
    const connection = await pool.getConnection();
    await connection.execute(
      'INSERT INTO login_attempts (ip_address, username, success) VALUES (?, ?, ?)',
      [attempt.ip, attempt.username || null, attempt.success]
    );
    connection.release();
  } catch (error) {
...
export const checkBruteForce = async (ip: string, username?: string): Promise<boolean> => {
  try {
    const connection = await pool.getConnection();
    ...
    if (username) {
      const [userAttempts] = await connection.execute(
        `SELECT COUNT(*) as count FROM login_attempts 
         WHERE username = ? AND success = FALSE 
         AND attempted_at > DATE_SUB(NOW(), INTERVAL 15 MINUTE)`,
        [username]
      );
      userFailedAttempts = (userAttempts as any[])[0].count;
    }
    
    connection.release();
    
    // Limite: 5 tentatives par IP ou 3 par utilisateur
    return (failedAttempts as any[])[0].count >= 5 || userFailedAttempts >= 3;
  } catch (error) {
...
export const authenticateUser = async (username: string, password: string, ip: string): Promise<{ success: boolean; user?: User; token?: string; message?: string }> => {
  try {
...
export const verifyToken = (token: string): { valid: boolean; user?: User; message?: string } => {
  try {
