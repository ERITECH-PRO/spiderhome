...
export const createDefaultAdmin = async () => {
    try {
        ...
        const [existingAdmin] = await connection.execute('SELECT id FROM users WHERE username = ?', ['admin_spiderhome']);
        if (existingAdmin.length === 0) {
            // Créer l'admin par défaut
            const hashedPassword = await bcrypt.hash('Industrial2024', 12);
            await connection.execute('INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)', ['admin_spiderhome', hashedPassword, 'admin']);
